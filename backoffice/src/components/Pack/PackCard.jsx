...
import { Package, CalendarDays, DollarSign, Pencil, Trash2, CheckCircle, XCircle } from 'lucide-react';

export default function PackCard({ item, onEdit, onDelete, canEdit, canDelete }) {
  return (
    <div className="tw-bg-white tw-rounded-xl tw-shadow-md hover:tw-shadow-lg tw-transition-shadow tw-duration-300 tw-border tw-border-gray-200 tw-flex tw-flex-col">
      ...
      {(canEdit || canDelete) && (
        <div className="tw-p-3 tw-bg-gray-50 tw-border-t tw-border-gray-100 tw-flex tw-justify-end tw-gap-2">
          {canEdit && <button onClick={onEdit} className="btn btn-icon btn-sm btn-primary-transparent rounded-pill" title="Modifier"><Pencil size={16} /></button>}
          {canDelete && <button onClick={onDelete} className="btn btn-icon btn-sm btn-danger-transparent rounded-pill" title="Supprimer"><Trash2 size={16} /></button>}
        </div>
      )}
    </div>
  );
