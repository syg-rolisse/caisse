...
function AuthContainer() {
  ...
  const activeAccount = useMutation(
      ...
      ),
    {
      ...
      onError: (error) => {
        ...
        toast.error(errorMessage || "Erreur lors de l’activation du compte.", {
          duration: 5000,
        });
      },
    }
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      // Stocke dans le state (utile pour reset-password)
      ...
      if (renderParam === "register" && tokenParam && emailParam && userIdParam) {
        navigate(window.location.pathname, { replace: true }); // Nettoie l’URL
