...
function DeleteSortie({ currentDeleteSortieId, refreshSortie }) {
  const prevSortieIdRef = useRef();
  const sortieLinkRef = useRef();
  // eslint-disable-next-line no-unused-vars
  const [isConnected, setIsConnected] = useState(false);
  const socket = useContext(SocketContext);
  const user = JSON.parse(localStorage.getItem("user"));
  const deleteSortie = useMutation(
    (param) =>
      axiosInstance.delete(
        `${
          import.meta.env.VITE_BACKEND_URL
        }/api/v1/sortie?sortieId=${param}&userConnectedId=${user?.id}`
      ),
    {
      onSuccess: (response) => {
        ...
        sortieLinkRef.current.click();
        if (socket && socket.connected) {
          socket.emit("sortie_deleted");
        }
      },
  ...
  const deleteEntrie = () => {
    // Lancer la mutation pour supprimer le sortie
    if (currentDeleteSortieId) {
      deleteSortie.mutate(currentDeleteSortieId);
    }
  };
  ...
  useEffect(() => {
    if (
      currentDeleteSortieId &&
      currentDeleteSortieId !== prevSortieIdRef.current
    ) {
      prevSortieIdRef.current = currentDeleteSortieId;
      if (sortieLinkRef.current) {
        sortieLinkRef.current.click();
      }
    }
  }, [currentDeleteSortieId]);
...
DeleteSortie.propTypes = {
  currentDeleteSortieId: PropTypes.number,
