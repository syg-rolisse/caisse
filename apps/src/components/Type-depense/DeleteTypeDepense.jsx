...
function DeleteTypeDepense({ currentDeleteTypeDepenseId, refreshTypeDepense }) {
  ...
  const deleteTypeDepense = useMutation(
    (param) =>
      axiosInstance.delete(
        `${
          import.meta.env.VITE_BACKEND_URL
        }/api/v1/type_depense?typeDepenseId=${param}`
      ),
    {
      onSuccess: (response) => {
        ...
        if (socket && socket.connected) {
          socket.emit("type_depense_deleted");
        }
      },
      ...
      onError: (error) => {
        ...
        if (validationErrors && Array.isArray(validationErrors)) {
          ...
          toast.error( error?.response?.data?.error ||
            ...
            error?.response?.data, { duration: 12000 });
        }
      },
    }
  );
  ...
  const deleteEntrie = () => {
    ...
    if (currentDeleteTypeDepenseId) {
      deleteTypeDepense.mutate(currentDeleteTypeDepenseId);
    }
  };
  ...
  useEffect(() => {
    if (
      currentDeleteTypeDepenseId &&
      currentDeleteTypeDepenseId !== prevTypeDepenseIdRef.current
    ) {
      prevTypeDepenseIdRef.current = currentDeleteTypeDepenseId;
...
DeleteTypeDepense.propTypes = {
  currentDeleteTypeDepenseId: PropTypes.number,
  refreshTypeDepense: PropTypes.func,
};

export default DeleteTypeDepense;
