              ...
              <div className="d-sm-flex mb-4 justify-content-between">
                ...
                <div className="d-flex gap-2 mt-sm-0 tw-w-full">
                  <div className="tw-w-full max-sm:tw-mt-2 tw-ml-3">
                    <input className="form-control form-control-xl" type="text" placeholder="Rechercher ici..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                  </div>
                </div>
              </div>
                ...
                <table className="customTable table table-bordered text-nowrap mb-0">
                  <thead>
                    <tr>
                      ...
                      {hasAnyActionPermission && <th className="fw-bold tw-text-center">Actions</th>}
                    </tr>
                  </thead>
                  <tbody>
                    {isLoading && (<tr><td colSpan={hasAnyActionPermission ? "6" : "5"} className="text-center py-5"><Spinner /></td></tr>)}
                    {isError && (<tr><td colSpan={hasAnyActionPermission ? "6" : "5"} className="text-center py-5"><div className="flex flex-col items-center gap-2 text-red-500"><ServerCrash className="w-8 h-8" /><span>{error?.message || "Impossible de charger les données."}</span></div></td></tr>)}
                    {!isLoading && !isError && filteredDepenses.length === 0 && (<tr><td colSpan={hasAnyActionPermission ? "6" : "5"} className="text-center"><span className="tw-text-gray-500">Aucune dépense trouvée</span></td></tr>)}
                    {!isLoading && !isError && filteredDepenses.map((depense) => (
                      <Fragment key={depense.id}>
                        <tr className={`${depense.bloquer ? 'tw-bg-gray-100' : ''} ${depense.rejeter ? 'tw-bg-red-50' : ''}`}>
                          <td>
                        ...
                        {expandedRows[depense.id] && (
                           <tr>
                            <td colSpan={hasAnyActionPermission ? "6" : "5"} className="p-0" style={{ backgroundColor: '#fafafa', borderBottom: '2px solid #dee2e6' }}>
                                    ...
                                    {depense.Mouvements.map(mouvement => (
                                      <tr key={mouvement.id}>
                                        ...
                                        {can('payeDepense') && (
                                            ...
                                            <button onClick={() => { setCurrentDepense(depense); setCurrentMouvement(mouvement); setShowCreateSortieModal(true); }} className="btn btn-icon btn-sm btn-primary-transparent" title="Modifier ce paiement" disabled={depense.bloquer || depense.rejeter}>
                                                <i className="ri-edit-line"></i>
                                            </button>
                                            <button onClick={() => { setCurrentMouvement(mouvement); setShowDeleteSortieModal(true); }} className="btn btn-icon btn-sm btn-danger-transparent ms-2" title="Supprimer ce paiement" disabled={depense.bloquer || depense.rejeter}>
